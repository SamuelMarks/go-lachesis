		...
		*/
		if ev.Round == -1 {
			...
			/*
				...
				reprocessed.
			*/
			if !roundInfo.Queued && roundNumber >= p.LastConsensusRound {
...
func (p *Poset) ProcessDecidedRounds() error {
	...
	for _, r := range p.PendingRounds {
		...
		if r.Index == p.LastConsensusRound {
			continue
		}
